	...
	$.fn.PavMegaMenuList = function(opts) {
		...
		this.each(function() {  
	 		$(".quickedit",this).click( function(){  
	 			location.href=config.action+"&id_ptsmegamenu="+$(this).attr('rel').replace("id_","");
	 		} );

	 		$(".quickdel",this).click( function(){  
	 			if( confirm(config.confirm_del) ){
	 				location.href=config.action+"&dodel=1&id_ptsmegamenu="+$(this).attr('rel').replace("id_","");
	 			}
...
jQuery(document).ready(function(){
 	...
 	$(".pts-modal , .ptsmegamenu .edit, .pts-modal-action").fancybox({
	 	'type':'iframe',
	 	'width':980,
	 	'height':500,
	 	afterLoad:function(   ){
...
jQuery(document).ready(function(){
    ...
    $(".ptsmegamenu td").attr('onclick','').unbind('click');
