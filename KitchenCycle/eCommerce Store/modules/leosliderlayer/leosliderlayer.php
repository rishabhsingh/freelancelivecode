...
class LeoSliderLayer extends Module {
    ...
    public function install() {
        ...
        /* Adds Module */
        if ($tab->add() && parent::install() && $this->registerHook('displaySlideshow') && $this->registerHook('displayHome') 
			&& $this->registerHook('displayTop') && $this->registerHook('displayTopColumn') && $this->registerHook('actionShopDataDuplication') && $this->registerHook('header') 
			&& Configuration::updateValue('LEOSLIDERLAYER_GROUP_DE', '1')) {
            $res = true;
    ...
    public function getSliderFieldsValues() {
        ...
        foreach ($languages as $lang) {
            ...
            if ($slide->video) {
                if($slide->video[(int) $lang['id_lang']] && is_array(Tools::jsonDecode(base64_decode($slide->video[(int) $lang['id_lang']]), true)))
...
//                    ),
	...
	public function hookHeader(){
		if (!isset($this->context->controller->php_self) || $this->context->getMobileDevice() != false) return ;
    ...
    public function headerHTML() {
        ...
        if (file_exists(_PS_THEME_DIR_ . 'css/modules/leosliderlayer/css/typo.css')) {
            $this->context->controller->addCSS(_THEME_CSS_DIR_ . 'modules/leosliderlayer/css/typo.css');
        } else {
